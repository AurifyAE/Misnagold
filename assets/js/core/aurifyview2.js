        ...
        for (const data of tableData) {
            ...
            setInterval(async () => {
                ...
                if (weight === "GM") {
                    ...
                    newRow.querySelector("#sellAED").innerText = parseFloat((parseFloat(goldAskingPrice) * unitInput * unitMultiplier * (purityInput / Math.pow(10, purityInput.length)) + parseFloat(sellPremium)).toFixed(2));
                    newRow.querySelector("#buyAED").innerText = (parseFloat(goldBiddingPrice) * unitInput * unitMultiplier * (purityInput / Math.pow(10, purityInput.length)) + parseFloat(buyPremium)).toFixed(2);
                } else {
                    // Update the sellAED and buyAED values for the current row
                    const sellAEDValue = parseFloat((parseFloat(goldAskingPrice) * unitInput * unitMultiplier * (purityInput / Math.pow(10, purityInput.length)) + parseFloat(sellPremium)).toFixed(4));
                    const buyAEDValue = parseInt(parseFloat(goldBiddingPrice) * unitInput * unitMultiplier * (purityInput / Math.pow(10, purityInput.length)) + parseFloat(buyPremium)).toFixed(0);
